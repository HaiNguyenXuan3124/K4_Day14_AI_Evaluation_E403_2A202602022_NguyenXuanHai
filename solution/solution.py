...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = f"""Score the response against every rubric criterion.
Return only a JSON object mapping each criterion to a number from 0.0 to 1.0.
Do not reward verbosity; score correctness and coverage of the requested task.
...
"""
        ...
        try:
            ...
            if isinstance(parsed, dict):
                ...
                parsed_scores = (
                    nested_scores if isinstance(nested_scores, dict) else parsed
                )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        def average_score(item: dict[str, Any]) -> float | None:
        ...
        item_averages = [average_score(item) for item in scores_batch]
        valid_averages = [score for score in item_averages if score is not None]
        overall_average = (
            sum(valid_averages) / len(valid_averages) if valid_averages else 0.5
        )

        first_position_scores = [
            score
            for item, score in zip(scores_batch, item_averages)
            if item.get("position") == 1 and score is not None
        ]
        later_position_scores = [
            score
            for item, score in zip(scores_batch, item_averages)
            if item.get("position") not in (None, 1) and score is not None
        ]

        positional_bias = False
        if first_position_scores and later_position_scores:
            positional_bias = (
                sum(first_position_scores) / len(first_position_scores)
                > sum(later_position_scores) / len(later_position_scores) + 0.05
            )
        elif len(valid_averages) >= 2:
            positional_bias = valid_averages[0] > (
                sum(valid_averages[1:]) / len(valid_averages[1:]) + 0.05
            )

        return {
            "positional_bias": positional_bias,
            "leniency_bias": bool(valid_averages and overall_average > 0.8),
            "severity_bias": bool(valid_averages and overall_average < 0.3),
        }
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_recall": (
                average(context_recall_values) if context_recall_values else None
            ),
            "avg_context_precision": (
                average(context_precision_values)
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        Compute average per metric, compare, list regressions, and set passed flag.
        """
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest_metrics = [
            ...
            if abs(score - lowest_score) <= 1e-12
        ]
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = (
                ...
                else "Review the trace and implement the root-cause fix"
            )
            lines.append(
                "| "
                + " | ".join(
                    [
                        f"F{index:03d}",
                        table_cell(failure.failure_type or "unknown"),
                        table_cell(self.find_root_cause(failure)),
                        table_cell(suggestion),
                        "Open",
                    ]
                )
                + " |"
    ...
    def generate_improvement_suggestions(
        ...
        for failure in failures:
            category = (failure.failure_type or "unknown").casefold()
            category_counts[category] = category_counts.get(category, 0) + 1

        category_suggestions = {
            "hallucination": (
                "Add grounding checks that reject claims unsupported by retrieved "
                "context"
            ),
            "irrelevant": (
                "Improve intent routing and prompt instructions so answers address "
                "the user question"
            ),
            "off_topic": (
                "Add intent-focused few-shot examples and query rewriting for "
                "off-topic answers"
            ),
            "incomplete": (
                "Improve retrieval coverage and require generation to include key "
            ...
            "refusal": (
                "Calibrate safety guardrails with allowed support examples to reduce "
                "incorrect refusals"
            ),
            "unknown": (
                "Inspect retrieval and generation traces to classify uncategorized "
                "failures"
            ),
        }
        suggestions = [
            category_suggestions.get(category, category_suggestions["unknown"])
        ...
        generic_suggestions = [
            ...
            "Track retrieval and answer metrics by difficulty and failure category",
